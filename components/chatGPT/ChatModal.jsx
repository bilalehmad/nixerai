...
import TypingAnimation from '../others/TypingAnimation';

const ChatModal = ({setIsOpen,prompt,setPrompt,handleGPTaction,chatLog,isOpen,heading,RegenerateResponce}) => {
    ...
    const autoSize = () => {
        ...
        const btn = document.getElementById('responceBtn');

            textarea.style.height = '26px'; // Reset the height to auto
            btn.classList.remove('bottom-28');
            btn.classList.add('bottom-20');
            ...
            if(textarea.scrollHeight > 100)
            {
                ...
                btn.classList.remove('bottom-20');
                    ...
                    <div className='flex flex-col h-[500px] '>
                                ...
                                <div key={index} className={`px-6 text-sm flex ${message.type == 'user' ? ' bg-white': ' bg-gray-50 '} p-4 gap-4 text-base md:gap-6 md:max-w-2xl lg:max-w-[38rem] xl:max-w-3xl md:py-6 lg:px-0 m-auto border-b border-black/10 `}>
                                        ...
                                        {message.type == 'bot' ? (
                                            <div className='relative left-2'>
                                                <div className='relative p-1 rounded-sm h-[30px] w-[30px] text-white flex items-center justify-center bg-[#1bc37d]'>
                        ...
                        <form className='flex-none '>
                            <div className="flex items-center max-w-[800px] mx-auto mb-2 w-full relative text-white border border-gray-300 rounded-lg">
                                ...
                                <button aria-label="Send" onClick={handleGPTaction} className="z-50 p-1 absolute rounded-md text-gray-400 enabled:bg-transparent enabled:hover:text-gray-400 bg-none transition-colors disabled:hover:bg-transparent right-1 md:right-2 disabled:opacity-40">
